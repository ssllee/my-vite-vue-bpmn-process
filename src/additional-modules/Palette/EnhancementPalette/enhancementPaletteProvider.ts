...
class EnhancementPaletteProvider extends PaletteProvider {
  ...
  getPaletteEntries() {
    ...
    function createMySqlTask(event) {
      const mySqlTask = elementFactory.createShape({ type: 'miyue:MySqlTask' })
      create.start(event, mySqlTask)
    }
    ...
    assign(actions, {
      ...
      'create.my-sql-task': {
        group: 'activity',
        // className: 'my-sql-task',
        className: 'bpmn-icon-intermediate-event-catch-condition',
        title: translate('My Sql Task'), // '数据库任务-my',
        action: {
          click: createMySqlTask,
          dragstart: createMySqlTask
        }
      },
      'task-separator': {
        group: 'activity',
