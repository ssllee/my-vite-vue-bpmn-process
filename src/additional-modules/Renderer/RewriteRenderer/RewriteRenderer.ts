...
class RewriteRenderer extends BaseRenderer {
  ...
  constructor(
    ...
    const handlers = (this.handlers = {
      ...
      // 自定义节点的绘制 MySqlTask
      'miyue:MySqlTask': function (parentGfx, element, attr) {
        // 渲染外层边框
        const attrs = {
          fill: getFillColor(element, defaultFillColor),
          fillOpacity: defaultTaskOpacity,
        ...
        svgAttr(customIcon, {
          ...
          width: 26,
          height: 26,
          href: './icons/custom-a.png'
        })
        svgAppend(parentGfx, customIcon)

        console.log(element, element.businessObject.name, attr)

        if (element.businessObject.name !== void 0) {
          const customText = svgCreate('text')
          svgAttr(customText, {
            ...(attr || {}),
            x: 10,
            y: 50, // y取的是父元素的y+height+20
            'font-size': '14',
            fill: '#000'
          })
          customText.innerHTML = element.businessObject.name
          svgAppend(parentGfx, customText)
        }
        return customIcon
      }
    })
